...
\code{Void} (there is no corresponding type in Nim) and the unique
...
Nim and the unit type \code{()} in Haskell. It's a type that has only
one value --- implicit in Nim and explicit in Haskell, denoted by
...
\src{snippet08}
In Nim, we would use generic functions, for instance:

\begin{snip}{nim}
func fst[A, B](p: (A, B)): A =
  return p[0]
\end{snip}
...
types: it's a tagged union of two types. Nim supports unions, but they
are not tagged. It means that in your program you have to somehow keep
track which member of the union is valid. This is horribly dangerous and
so unions are relegated to the \code{\{.union.\}} pragma (pragmas in Nim
provide for using experimental features without special syntax).
To create a tagged union, you have to define a tag --- an enumeration ---
and combine it with a \code{case} expression to form an object variant.
For instance, an object variant of an \code{int} and a \code{string} could
be implemented as:

\begin{snip}{nim}
type ContactKind = enum
  isPhone, isEmail
type Contact = object
...
\begin{enumerate}
  ...
  \item
        ...
        \begin{snip}{nim}
func i(n: int): int = n
func j(b: bool): int = if b: 0 else: 1
\end{snip}
