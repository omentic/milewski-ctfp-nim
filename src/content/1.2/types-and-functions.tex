...
except that there is a new technology called \newterm{good language design}
that lets the programmer use a language other than C/C++ that has a proper
type system and solid type inference. Such as Rust: or more relevantly, Nim.
...
that corresponds to machine type, just like Nim's \code{int}.
...
The things we call functions in C++ or most other imperative languages,
...
\src{snippet10}
In Nim you would write this function as:

\begin{snip}{cpp}
func unit[T](_: T) = discard
\end{snip}

The \code{discard} statement is typically used to ``discard'' a value from an
expression: as Nim ensures that all expressions returning a type must be dealt
with. This also allows for the \code{return} keyword to be elided in some
cases. However, as is the case here, \code{discard} can be used on its own as
a no-op. The \code{_} identifier is a similar no-op: it discards what would
normally be a binding.

Next in the typology of types is a two-element set. In Nim it's called
\code{bool} and in Haskell, predictably, \code{Bool}. In Haskell it
...
Nim's boolean type is also defined as an enumeration:
\begin{snip}{nim}
type bool = enum
  false = 0, true = 1
\end{snip}
...
\code{isAlpha} or \code{isDigit}. In Nim there is similar functionality split
between \code{std/strutils} and \code{std/unicode}.
